...
class MusicConverter(models.Model):
    ...
    bitrate = fields.Integer(
        'Bitrate', readonly=True, states={'draft': [('readonly', False)]}, default=0)
    ...
    def action_cancel(self):
        ...
        self.env.cr.execute('''
            UPDATE oomusic_converter_line SET state = 'cancel' WHERE converter_id IN %s
        ''', (tuple(self.ids),))
    ...
    def convert(self):
        with api.Environment.manage():
            with self.pool.cursor() as cr:
                ...
                # Avoid upsampling
                if fn_ext[1:] == transcoder.output_format.name\
                        and (
